...
void OLED_WriteCommand(uint8_t Command)
{
    ...
    ESP_ERROR_CHECK(i2c_master_transmit(oled_dev_handle, writebuffer, 2, 100));
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    ESP_ERROR_CHECK(i2c_master_transmit(oled_dev_handle, writebuffer, Count+1, 100));
}
...
void OLED_Init(int port, uint8_t add, int scl, int sda, int speed)
{
    ...
    //总线复位，清除可能残留的stuck状态
    ESP_ERROR_CHECK(i2c_master_bus_reset(oled_bus_handle));
    ...
    //探测OLED是否应答
    esp_err_t probe_ret = i2c_master_probe(oled_bus_handle, add >> 1, 200);
    if (probe_ret != ESP_OK) {
        printf("OLED NOT FOUND at 7-bit:0x%02X (8-bit:0x%02X), err:%s\n",
               add >> 1, add, esp_err_to_name(probe_ret));
        printf("Check: VCC→3.3V GND→GND SCL→GPIO%d SDA→GPIO%d\n", scl, sda);
        ESP_ERROR_CHECK(probe_ret);  // 会 abort，防止后续死循环
    }
...
/* ── 默认配置 ── */
#define OLED_DEFAULT_PORT   I2C_NUM_0
#define OLED_DEFAULT_ADDR   0x78      /* 8-bit 地址 */
#define OLED_DEFAULT_SPEED  100000    /* 100kHz */

/**
  * 函    数：OLED初始化（简化版，只需传引脚）
  * 参    数：scl  SCL引脚号
  * 参    数：sda  SDA引脚号
  * 返 回 值：无
  * 说    明：I2C端口、地址、速度使用默认值，如需自定义请用 OLED_Init
  */
void OLED_Init_Pins(int scl, int sda)
{
    OLED_Init(OLED_DEFAULT_PORT, OLED_DEFAULT_ADDR, scl, sda, OLED_DEFAULT_SPEED);
}
